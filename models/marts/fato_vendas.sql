...
vendas_join as (
   ...
   left join dim_clientes as c 
       on vp.ID_CLIENTE = c.ID_CLIENTE
   left join dim_localidade as l 
       on vp.ID_ENDERECO_COBRANCA = l.ID_ENDERECO
   left join dim_produto as p 
       on vp.ID_PRODUTO = p.ID_PRODUTO
   left join dim_cartao as cc 
       on vp.ID_CARTAO_CREDITO = cc.ID_CARTAO
   left join dim_data as t 
       on vp.DATA_PEDIDO = t.DATA_PEDIDO
),
fato_vendas as (
   select
